...
export class ListeComponent implements OnInit {
  displayedColumns: string[] = ['numero', 'client', 'date_emission', 'date_echeance', 'montant_total', 'statut', 'actions'];
  factures: Facture[] = [];
  loading = false;
  searchTerm = '';
  totalItems = 0;
  ...
  loadFactures(): void {
    ...
    this.facturesService.getAll(params).subscribe({
      next: (response) => {
  ...
  onEdit(facture: Facture): void {
    this.router.navigate(['/factures-clients/edit', facture.id]);
  }
  ...
  onDelete(facture: Facture): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.facturesService.delete(facture.id).subscribe({
          next: () => {
  ...
  onCreate(): void {
    this.router.navigate(['/factures-clients/nouvelle']);
  }

  onDownloadPDF(facture: Facture): void {
    this.facturesService.getPDF(facture.id).subscribe({
      next: (blob) => {
  ...
  canEdit(facture: Facture): boolean {
    return facture.statut === 'Brouillon';
  }

  canDelete(facture: Facture): boolean {
    return facture.statut === 'Brouillon';
  }
